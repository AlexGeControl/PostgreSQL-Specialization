...
class SWAPIScraper:
    def __init__(self, root_url, max_workers, redis_host, redis_port):
        ...
        # Redis keys
        self.PENDING_QUEUE = "swapi:pending"  # Now a sorted set instead of list
        self.COMPLETED_SET = "swapi:completed"
        self.FAILED_SET = "swapi:failed"
        self.DATA_PREFIX = "swapi:data:"
        ...
        self.session.headers.update(
            {"User-Agent": "SWAPI-Scraper/1.0 (Educational Purpose)"}
        )
    ...
    def run(self, seed_urls: Iterable[str]):
        ...
        logger.info("Starting SWAPI scraper...")
        ...
        with ThreadPoolExecutor(
            max_workers=self.max_workers, thread_name_prefix="SWAPIWorker"
        ) as executor:
            # Submit worker tasks
            futures = [
...
class SWAPIPostgreSQLConnector:
    """
    PostgreSQL connector class for managing SWAPI data ingestion.

    This class handles:
    - Database connection management
    - Table creation and schema management
    - Batch insertion of scraped SWAPI data
    ...
    def create_table(self, table_name: str) -> None:
        """
        Creates the table for SWAPI data in the PostgreSQL database.

        The table schema includes:
        - id: Auto-incrementing primary key
        - url: The original SWAPI URL (unique constraint)
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Create new table with optimized schema for SWAPI data
                create_stmt = f"""
            ...
            with conn.cursor() as cur:
                try:
                    for entity in entities:
                        stmt = f"INSERT INTO {table_name} (url, data) VALUES (%s, %s) RETURNING id;"
                        cur.execute(stmt, (entity["url"], json.dumps(entity["data"])))
                        result = cur.fetchone()
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    f"""
                SELECT 
                    REGEXP_REPLACE(url, 'https://swapi\\.py4e\\.com/api/([^/]+)/.*', '\\\\1') as resource_type,
                    COUNT(*) as count
                FROM {table_name} 
                ...
                resource_distribution = dict(cur.fetchall())
...
def parse_command_line_args():
    ...
    parser = argparse.ArgumentParser(
        description="SWAPI Web Scraper - Scrape Star Wars API data with Redis caching",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Scrape single URL
  python index-swapi.py https://swapi.py4e.com/api/films/1/
  
  # Scrape multiple URLs
  python index-swapi.py https://swapi.py4e.com/api/films/1/ https://swapi.py4e.com/api/people/1/
  
  # Use default SWAPI root URL
  python index-swapi.py
        """,
    )
    ...
    parser.add_argument(
        ...
        help="One or more SWAPI URLs to scrape. If none provided, defaults to the 1st URL of SWAPI films.",
    )
    ...
    if not args.urls:
        args.urls = ["https://swapi.py4e.com/api/films/1/"]
...
def main():
    ...
    # Create scraper
    scraper = SWAPIScraper(
        root_url="https://swapi.py4e.com/api/",
    ...
    pg_connector = SWAPIPostgreSQLConnector(batch_size=100)
