...
def postgres_local() -> ConnectionParams:
    """Returns connection parameters for the local PostgreSQL database from Docker Compose
    ...
    return {
        "host": "localhost",
        "port": 5432,
        "database": "postgres",  # Updated to match Docker Compose
        "user": "postgres",
        "pass": "postgres",
    }
...
def postgres_sqlalchemy_connection_string(secrets: ConnectionParams) -> str:
    ...
    return f"postgresql://{secrets['user']}:{secrets['pass']}@{secrets['host']}:{secrets['port']}/{secrets['database']}"


if __name__ == "__main__":
    # Unit tests for PostgreSQL connection strings
    assert (
        psycopg_connection_string(postgres_read_write())
        == "dbname=pg4e_b0429fdfdf user=pg4e_b0429fdfdf password=pg4e_p_63e777959d7b58a host=pg.pg4e.com port=5432"
    ), "Psycopg connection string is incorrect"
    print("[Psycopg Connection String Check]: Passed")
    assert (
        postgres_sqlalchemy_connection_string(postgres_read_write())
        == "postgresql://pg4e_b0429fdfdf:pg4e_p_63e777959d7b58a@pg.pg4e.com:5432/pg4e_b0429fdfdf"
    ), "PostgreSQL SQLAlchemy connection string is incorrect"
